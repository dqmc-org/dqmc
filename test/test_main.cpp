...
int main(int argc, char* argv[]) {
  ...
  // std::cout << cubic->Index2Momentum( cubic->GammaPointIndex() ) <<
  // std::endl; std::cout << cubic->Index2Momentum( cubic->XPointIndex() ) <<
  // std::endl; std::cout << cubic->Index2Momentum( cubic->MPointIndex() ) <<
  // std::endl; std::cout << cubic->Index2Momentum( cubic->RPointIndex() ) <<
  // std::endl; std::cout << cubic->Index2Momentum( cubic->DeltaLineIndex() ) <<
  // std::endl;
  ...
  //     boost::posix_time::second_clock::local_time(); std::cout <<
  //     boost::format(" Current time: %s \n") % current_time << std::endl;
  // }
  ...
  //     boost::format fmt_mpi(" Distribute tasks to %s processes, with the
  //     master process being %s. \n"); std::cout << fmt_mpi % world.size() %
  ...
  //     *walker, *meas_handler, *checkerboard );
  // }
  // else {
  //     QuantumMonteCarlo::DqmcInitializer::initial_modules( *model, *lattice,
  //     *walker, *meas_handler );
  ...
  //     std::endl;
  // }
  // // QuantumMonteCarlo::DqmcIO::read_bosonic_fields_from_file( "../out.dat",
  // *model);

  // // initialize modules for dqmc
  // QuantumMonteCarlo::DqmcInitializer::initial_dqmc( *model, *lattice,
  // *walker, *meas_handler );
  ...
  // QuantumMonteCarlo::Dqmc::thermalize( *walker, *model, *lattice,
  // *meas_handler ); QuantumMonteCarlo::Dqmc::measure( *walker, *model,
  // *lattice, *meas_handler );
  ...
  // // QuantumMonteCarlo::DqmcIO::output_imaginary_time_grids( std::cout,
  // *walker );
  ...
  //             std::cout,
  //             meas_handler->find<Observable::ScalarObs>("equaltime_sign") );
  //     }

  //     if ( meas_handler->find("dynamic_sign") ) {
  //         QuantumMonteCarlo::DqmcIO::output_observable(
  //             std::cout,
  //             meas_handler->find<Observable::ScalarObs>("dynamic_sign") );
  //     }
  ...
  //         QuantumMonteCarlo::DqmcIO::output_observable(
  //             std::cout,
  //             meas_handler->find<Observable::ScalarObs>("filling_number") );
  //     }
  ...
  //         QuantumMonteCarlo::DqmcIO::output_observable(
  //             std::cout,
  //             meas_handler->find<Observable::ScalarObs>("kinetic_energy") );
  //     }
  ...
  //         QuantumMonteCarlo::DqmcIO::output_observable(
  //             std::cout,
  //             meas_handler->find<Observable::ScalarObs>("spin_density_structure_factor")
  //             );
  //     }
  ...
  //     // important!!!
  //     // todo: the correctness of superfluid stiffness measurements should be
  //     further checked in attractive hubbard model if (
  //     meas_handler->find("superfluid_stiffness") ) {
  //         QuantumMonteCarlo::DqmcIO::output_observable(
  //             std::cout,
  ...
  // std::cout << " Initialization finished. \n\n"
  //           << " The simulation is going to get started with parameters shown
  //           below : \n\n"
  //           << std::endl;
  ...
  // meas_handler->set_measure_params(sweeps_warmup, bin_num, bin_size,
  // sweeps_between_bins); meas_handler->set_observables(obs_list);
  ...
  //     QuantumMonteCarlo::DqmcInitializer::set_measured_momentum(*meas_handler,
  //     lattice->MPointIndex());
  //     QuantumMonteCarlo::DqmcInitializer::set_measured_momentum_list(*meas_handler,
  //     lattice->kStarsIndex());
  // }
  ...
  // QuantumMonteCarlo::DqmcInitializer::initial_modules(*lattice, *model,
  // *walker, *meas_handler);

  // // using checkerboard break-up
  // // CheckerBoard::CheckerBoardBase* checkerboard = new
  // CheckerBoard::Square();
  // // QuantumMonteCarlo::DqmcInitializer::initial_modules(*lattice, *model,
  // *walker, *meas_handler, *checkerboard);
  ...
  // QuantumMonteCarlo::DqmcInitializer::initial_dqmc(*lattice, *model, *walker,
  // *meas_handler);
  ...
  // // QuantumMonteCarlo::Dqmc::sweep_forth_and_back(*walker, *model, *lattice,
  // *meas_handler);
  ...
  // //     std::cout << obs.name() << "  " << obs.mean_value() << "  " <<
  // obs.error_bar() << std::endl;
  // // }
  ...
  // if (meas_handler->find("equaltime_sign")) {
  //     const auto obs =
  //     meas_handler->find<Observable::ScalarObs>("equaltime_sign"); std::cout
  //     << obs.name() << "  " << obs.mean_value() << "  " << obs.error_bar() <<
  //     std::endl;
  // }
  ...
  // if (meas_handler->find("local_spin_corr")) {
  //     auto obs =
  //     meas_handler->find<Observable::ScalarObs>("local_spin_corr"); std::cout
  //     << obs.name() << "  " << obs.mean_value() << "  " << obs.error_bar() <<
  //     std::endl;
  // }
  ...
  // // std::cout << meas_handler->find_scalar("filling_number").name() <<
  // std::endl;
  // // std::cout << meas_handler->find_matrix("greens_functions").name() <<
  // std::endl;
  ...
  // config["Model"]["type"].value_or("RepulsiveHubbard"); const auto
  // checker_board = config["CheckerBoard"]["whether_or_not"].value_or(true);
  ...
  // config["library"]["name"].value_or("sv");
  // // // std::string_view library_author =
  // config["library"]["authors"][0].value_or("sv");
  // // // int64_t depends_on_cpp_version =
  // config["dependencies"]["cpp"].value_or(0);
  ...
  // // std::cout << lattice->Index2Momentum(lattice->GammaPointIndex()) <<
  // std::endl;
  // // std::cout << lattice->Index2Momentum(lattice->XPointIndex()) <<
  // std::endl;
  // // std::cout << lattice->Index2Momentum(lattice->MPointIndex()) <<
  // std::endl;
  ...
  // Lattice::LatticeBase* lattice = new Lattice::Square();
  // QuantumMonteCarlo::DqmcWalker* walker = new
  // QuantumMonteCarlo::DqmcWalker(); Measure::MeasureHandler* meas_handler =
  // new Measure::MeasureHandler(); CheckerBoard::CheckerBoardBase* checkerboard
  // = new CheckerBoard::Square();
  ...
  // QuantumMonteCarlo::DqmcInitializer::initial_modules(*lattice, *model_cb,
  // *walker, *meas_handler, *checkerboard);
  // QuantumMonteCarlo::DqmcInitializer::initial_modules(*lattice,
  // *model_direct, *walker, *meas_handler);
  ...
  // Observable::ObservableHandler* handler = new
  ...
  // (handler->m_eqtime_scalar_obs[0])->measure(*meas_handler, *model,
  // *lattice);

  // // test utils

  // Utils::SvdStack* svd_stack = new Utils::SvdStack(4,10);
  // // Utils::FFTSolver::FFTSolver2d* solver = new
  ...
  // Observable::ObservableBase* obs = new
  // Observable::Observable<Observable::ScalarType>();

  // Observable::Observable<Observable::ScalarType>* casted_obs =
  // (dynamic_cast<Observable::Observable<Observable::ScalarType>*>(obs));
