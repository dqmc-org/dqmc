 ...
 *  which is designed to collect Observable::Observable classes among a set of
 * MPI processes.
 */
...
class MPI {
 public:
  ...
  // note that the Utils::MPI class should be a friend class of
  // Measure::MeasureHandler to get access to the protected observable members
  static void mpi_gather(const boost::mpi::communicator& world,
                         Measure::MeasureHandler& meas_handler) {
    // scalar observables
