...
int main(int argc, char* argv[]) {
  ...
  general.add_options()("help,h", "Show help message")(
      ...
      "output,o",
      po::value<std::string>(&out_path)->default_value("../example"),
      "Output folder path")("fields,f", po::value<std::string>(&fields_file),
                            "Path to auxiliary fields configuration file")(
      "run-id,r", po::value<int>(&run_id)->default_value(0),
      "Unique run identifier for parallel execution");

  po::options_description model_opts("Model options");
  model_opts.add_options()(
      "model.type",
      po::value<std::string>()->default_value("AttractiveHubbard"),
      "Model type (AttractiveHubbard, RepulsiveHubbard)")(
      "model.hopping_t", po::value<double>()->default_value(1.0),
      "Hopping parameter t")("model.onsite_u",
                             po::value<double>()->default_value(4.0),
                             "On-site interaction U")(
      "model.chemical_potential", po::value<double>()->default_value(0.0),
      "Chemical potential μ");

  po::options_description lattice_opts("Lattice options");
  lattice_opts.add_options()("lattice.type",
                             po::value<std::string>()->default_value("Square"),
                             "Lattice type (Square, Cubic, Honeycomb)")(
      "lattice.size",
      po::value<std::vector<int>>()->multitoken()->default_value({4, 4}, "4 4"),
      "Lattice dimensions");

  po::options_description checkerboard_opts("Checkerboard options");
  checkerboard_opts.add_options()("checkerboard.enable",
                                  po::value<bool>()->default_value(false),
                                  "Enable checkerboard decomposition");

  po::options_description mc_opts("Monte Carlo options");
  mc_opts.add_options()("mc.beta", po::value<double>()->default_value(8.0),
                        "Inverse temperature β")(
      "mc.time_size", po::value<double>()->default_value(160),
      "Imaginary time discretization")("mc.stabilization_pace",
                                       po::value<int>()->default_value(10),
                                       "QR decomposition frequency");

  po::options_description measure_opts("Measurement options");
  measure_opts.add_options()("measure.sweeps_warmup",
      ...
      "measure.sweeps_between_bins", po::value<int>()->default_value(20),
      "Sweeps between bins");
  ...
  observable_opts.add_options()(
      ...
      po::value<std::vector<std::string>>()->multitoken()->default_value(
          {"filling_number", "double_occupancy", "kinetic_energy",
           "momentum_distribution", "local_spin_corr",
           "spin_density_structure_factor", "charge_density_structure_factor",
           "s_wave_pairing_corr", "greens_functions", "density_of_states",
           "superfluid_stiffness", "dynamic_spin_susceptibility"},
          "filling_number double_occupancy ..."),
      "List of observables to measure");

  po::options_description momentum_opts("Momentum options");
  momentum_opts.add_options()(
      "momentum.point", po::value<std::string>()->default_value("MPoint"),
      "Momentum point (GammaPoint, MPoint, XPoint, RPoint)")(
      "momentum.list", po::value<std::string>()->default_value("KstarsAll"),
      "Momentum list (KstarsAll, DeltaLine, ZLine, etc.)");
  ...
  try {
    ...
    if (vm.count("config")) {
      ...
      if (ifs) {
        ...
        po::notify(vm);  // Update variables with config file values
      } else {
  ...
  DQMC::Initializer::parse_config(config, 1, model, lattice, walker,
