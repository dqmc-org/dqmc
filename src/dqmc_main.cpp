...
int main(int argc, char* argv[]) {
  ...
  if (rank == master) {
    ...
    boost::format fmt_mpi(
        ">> Distribute tasks to %s processors, with the master processor being "
        "%s.\n");
  ...
  QuantumMonteCarlo::DqmcInitializer::parse_toml_config(
      config_file, world.size(), model, lattice, walker, meas_handler,
      checkerboard);
  ...
  if (rank == master) {
    QuantumMonteCarlo::DqmcIO::output_init_info(std::cout, world.size(), *model,
                                                *lattice, *walker,
  ...
  if (rank == master) {
    ...
    const auto fields_out =
        (fields_file.empty()) ? out_path + "/fields.out" : fields_file;
