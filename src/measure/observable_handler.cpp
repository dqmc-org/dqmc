...
void ObservableHandler::initial(const ObsNameList& obs_list) {
  ...
  for (const auto& obs_name : obs_list) {
    ...
    if (obs_name == "momentum_distribution") {
      ...
      momentum_distribution->set_name_and_description(obs_name,
                                                      "Momentum distribution");
    ...
    if (obs_name == "local_spin_corr") {
      ...
      local_spin_corr->set_name_and_description(obs_name,
    ...
    if (obs_name == "s_wave_pairing_corr") {
      ...
      s_wave_pairing_corr->set_name_and_description(
          obs_name, "S-wave pairing correlation");
    ...
    if (obs_name == "density_of_states") {
      ...
      density_of_states->set_name_and_description(obs_name,
                                                  "Density of states");
    ...
    if (obs_name == "superfluid_stiffness") {
      ...
      superfluid_stiffness->set_name_and_description(obs_name,
                                                     "Superfluid stiffness");
    ...
    if (obs_name == "dynamic_spin_susceptibility") {
      ...
      dynamic_spin_susceptibility->set_name_and_description(
          obs_name, "Dynamic Spin Susceptibility");
  ...
  if (!this->m_eqtime_scalar_obs.empty() ||
    ...
    equaltime_sign->set_name_and_description("equaltime_sign",
                                             "Averaged sign (equal-time)");
  ...
  if (!this->m_dynamic_scalar_obs.empty() ||
    ...
    dynamic_sign->set_name_and_description("dynamic_sign",
                                           "Averaged sign (dynamical)");
