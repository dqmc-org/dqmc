...
RepulsiveHubbard::RepulsiveHubbard(RealScalar hopping_t, RealScalar onsite_u,
...
void RepulsiveHubbard::set_model_params(RealScalar hopping_t, RealScalar onsite_u,
                                        RealScalar chemical_potential) {
  DQMC_ASSERT(hopping_t >= 0.0);
  DQMC_ASSERT(onsite_u >= 0.0);
  this->m_hopping_t = hopping_t;
...
double RepulsiveHubbard::get_update_ratio(const Walker& walker, TimeIndex time_index,
...
void RepulsiveHubbard::update_greens_function(Walker& walker, TimeIndex time_index,
  ...
  green_tt_dn -= factor_dn * green_tt_dn.col(space_index) *
                 (Eigen::VectorXd::Unit(this->m_space_size, space_index).transpose() -
...
void RepulsiveHubbard::mult_invB_from_left(GreensFunc& green, TimeIndex time_index,
                                           Spin spin) const {
  // Multiply a dense matrix, specifically a greens function, from the left by
  // B(t)^-1
...
void RepulsiveHubbard::mult_invB_from_right(GreensFunc& green, TimeIndex time_index,
                                            Spin spin) const {
  // Multiply a dense matrix, specifically a greens function, from the right by
  // B(t)^-1
...
void RepulsiveHubbard::mult_transB_from_left(GreensFunc& green, TimeIndex time_index,
                                             Spin spin) const {
  // Multiply a dense matrix, specifically a greens function, from the left by
  // B(t)^T
